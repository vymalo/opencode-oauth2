...
import { createBunTransport, createEndpoint } from "../packages/opencode-browser/dist/lib.js";
...
const endpoint = await createEndpoint(
  ...
  { logger, createServerTransport: createBunTransport, createAgentSocket: bunAgentSocket }
);
...
Bun.serve({
  ...
  async fetch(req) {
    ...
    if (req.method === "POST" && url.pathname === "/cmd") {
      const { action, group, params, target } = await req.json();
      try {
        const data = await endpoint.send(action, group ?? "", params ?? {}, undefined, target);
