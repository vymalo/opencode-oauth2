...
| Dashboard shows *bridge rejected the token…* | The token the extension sent ≠ the token the **running host** holds. Two common causes: (1) your saved token is stale — copy the current one from the plugin log (`browser_bridge_token`) or [`bridge.json`](#the-bridge-state-file-bridgejson) and *Save & reconnect*; or (2) the token is fine but an **old host still owns the port** with a pre-rotation token (it resolved its token once at boot and never re-reads) — restart the process that owns the bridge port (`lsof -nP -iTCP:<port> -sTCP:LISTEN` to find it; often a long-lived IDE/desktop OpenCode). The host logs `browser_handshake_rejected` with non-secret `expected`/`got` fingerprints: **same length, different value ⇒ a different/rotated token (stale host), not a paste typo.** The extension no longer floods — it retries slowly and **auto-recovers** once a good host is back, so you don't have to manually reconnect after fixing the host. Pinning an explicit `token` in plugin options avoids the whole class. |
